...
**Primary build target:** Mobile-first (Expo / React Native)  
**Audience:** Product, design, engineering  
**Scope:** Phases 0–5 (Capture-first · Workbench · Micro-cycle · Cleaning · Collaboration · Formation)
...
## 1. What "good" feels like (design principles)

Choreographers — whether a hip-hop director, a contemporary teacher, or a ballet répétiteur who has spent thirty years defending the irreducibility of movement — are deeply allergic to tools that make their work feel like paperwork. They will not forgive an interface that asks them to translate their artistry into a system. They will love an interface that feels like a very good assistant: always in the room, never speaks unless asked, never loses anything, and knows exactly which version you meant when you say "the one we did on Tuesday."

Roam's job is not to structure creativity. It is to hold the things that creativity produces — the improvised phrase at 11pm, the note given to a dancer that was perfect and never repeated, the temperature of a section between one rehearsal and the next — and give them back at the right moment. The tool should disappear. What should remain is the work.

- **Notes are blank lines, not forms.** A choreographer writes "this section should feel like dragging something heavy through water" and that note lives at a timecode. No fields. No categories. No axes.
- **Clips feel like a shoebox of ideas, not a filing cabinet.** Retrieval is powerful; capture is frictionless.
- **Formation mapping feels like tape on a studio floor, not CAD.** Fast, physical, disposable, revisable.
- **Structure lives in infrastructure.** Search, retrieval, versioning, and alignment — never paperwork.

---

## 2. Simulation-validated fixes (v2.1)

Synthetic simulation across **200 sessions** (100 working choreographers, 100 dancer-makers) was run to validate product direction before committing to design and build.

### Summary metrics (v2.0 → v2.1)

| Metric | v2.0 | v2.1 | Δ |
|--------|------|------|---|
| Combined strong fit | ~42% | **~63%** | **+21pp** |
| Threshold target | — | **60%** ✓ | crossed |
| Dancer-makers strong fit | — | **~69%** | — |
| Professionals strong fit | — | **~57%** | — |
...
| App-switching (professionals) | ~76% | ~73% | structural; not eliminated |
| Share fear (dancers) | ~55% | ~55% | Phase 4 is load-bearing |

### Fix 1 — Capture-first entry

72% of dancer-maker sessions broke on the **music-first requirement**. The current flow (session → music → capture) is the wrong door for anyone who has movement before they have a song. V2.1 adds a capture-first entry path: the app opens to a single record button with zero setup required. The clip saves to an Inbox. Session, music, and tags can be added after.

### Fix 2 — Voice-first note pin

46% of professional sessions showed **note friction** — the user stopped to type, lost creative flow, and sometimes abandoned the note entirely. Notes are now voice-first: hold to record a memo, release to save. Text remains available as a secondary option. No categories, no fields. The thought lives at the timecode in the user's own voice.

### Build-order implication (from simulation)

The simulation confirms a strict build order:

- **Phase 0** proves the dancer-maker door: one-tap capture + Inbox (eliminate music-first barrier; reduce tool anxiety)
- **Phase 1** proves the professional loop: workbench + repetition tool (reduce fragmentation)
- **Phase 4** is **critical for dancer retention**: structured feedback container resolves share fear
...
#### 0.3 The first screen — two doors
...
#### 0.4 Capture-first flow — detailed requirements
...
#### 0.5 Returning user home screen
...
### Phase 1 — Session Workbench Redesign
...
### 1.4b  Stem Focus — Listening to One Layer at a Time

#### Problem statement

A choreographer listening to a full mix is hearing four or five things at once. That is useful for some tasks and actively harmful for others. When you are trying to count a section, the vocals are noise. When you are working on facial expression and emotional delivery, the drums are noise. When you are trying to understand where the music breathes — where the tension builds and where it releases — the drums and the bass both compete for attention. The solution is not a better waveform. It is the ability to hear what matters for the task at hand.

#### Goal

Let a choreographer mute or solo any individual stem of the song — vocals, drums, bass, instruments — during playback, without switching context or disrupting the timeline.

#### The stems

| Stem | Label | Primary choreographic use |
|------|-------|--------------------------|
| Vocals | 🎤 Vocals | Lyrical phrasing, emotional interpretation, facial expression timing, matching movement quality to melodic peaks and rests |
| Drums | 🥁 Drums | Counting, rhythmic structure, identifying accents, drilling timing precision |
| Bass | 🎸 Bass | Grounding and weight work, feeling the floor, identifying low-frequency pulses that don't show on a high-frequency waveform |
| Instruments | 🎹 Instruments | Harmonic context, chord texture, identifying sections that are rhythmically similar but tonally distinct |

#### Interface

Stem controls live on the music waveform track. They are hidden by default and revealed by a single tap on the waveform track header (the 🎵 icon). When expanded, four stem buttons appear inline below the waveform:
...
Each stem button has three states:

| State | Visual | Behavior |
|-------|--------|----------|
| **All active** (default) | All four buttons unfilled | Full mix plays — no processing applied |
| **Solo** | One button highlighted, others dimmed | Only the selected stem plays |
| **Muted** | Button crossed out | That stem is removed from the mix |

Interaction model:
- **Tap once** → solo that stem (all others mute)
- **Tap again** → return to full mix
- **Long-press** → toggle mute on that stem only (leaving others unchanged)
- Stems can be combined: long-press Drums to mute it, then long-press Bass to mute it — now you hear Vocals + Instruments only

#### Processing and availability

Stem separation runs as a background job when a track is added to a session (same pipeline as waveform generation). It uses Demucs v4 server-side. The user does not trigger it — it simply becomes available when ready.

If stems are still processing, the stem buttons are shown in a dimmed/disabled state with a subtle loading indicator. The track plays normally in full mix until stems are ready. Most tracks complete within 30–90 seconds of upload depending on duration.

If a track cannot be separated (edge cases: very short clips, corrupted files), the stem buttons remain hidden and no error is shown — the feature simply does not appear.

The feature is gated behind a `stem_focus_enabled` flag on `user_preferences` (default: false for new users, toggleable in Settings → Playback).

#### Acceptance criteria

- [ ] Stem controls are hidden by default and revealed in one tap on the waveform track header
- [ ] Tapping a stem button solos that stem within 200ms — no buffering pause
- [ ] Long-press mute toggles a single stem without affecting others
- [ ] Full mix playback is unaffected when no stem is active
- [ ] Stem controls are visually absent when processing is incomplete — no error state shown
- [ ] Transitioning between loop boundaries does not reset stem state
- [ ] Stem state persists for the session duration (closing and reopening the session restores last stem configuration)

#### Technical notes

- Stem separation: Demucs v4 server-side; results cached as `stems/{track_id}/{vocals|drums|bass|other}.m4a` in Supabase Storage
- `active_stems` stored as a bitmask or JSON array on `sessions` (`e.g. { vocals: true, drums: false, bass: true, other: true }`)
- Client mixes stems in real time using `AVAudioMixerNode` (iOS) / `MediaPlayer` audio session mixing (Android) — stems are pre-downloaded to device when the session is opened
- Stem download is background/lazy: full mix plays immediately; stems downloaded in parallel; UI updates to show stem controls when all four stems are cached locally
- `stem_focus_enabled` boolean on `user_preferences` (default: false); toggled in Settings

---

### 1.4c  Multi-Loop Regions and Continuous Recording

---

### 1.4d  Music Source Import — URL Paste

#### Problem statement

App-switching at 73% of professional sessions is the single unresolved structural friction in the simulation. Some of that switching is permanent — choreographers will always discover music on Spotify, YouTube, and SoundCloud. But the moment between "I found the track" and "it's in my Roam session" is where Roam can close the gap without requiring any platform partnership.

#### Goal

Let a choreographer paste a Spotify or YouTube URL directly into the music attachment flow. The app resolves it, extracts the audio, and attaches it to the session. From that point forward the experience is identical to a file upload.

#### Flow

The music attachment input — wherever it appears (new session setup, workbench "add music" action) — accepts either a file or a URL:
...
1. User pastes a URL → app detects the source (Spotify / YouTube / SoundCloud)
2. A preview loads: track name, artist, duration — fetched from the URL metadata
3. User confirms → extraction job begins in the background
4. A subtle progress indicator replaces the waveform placeholder while the track processes
5. When ready: waveform renders, section analysis runs — identical to a file upload from this point

If extraction fails (region restriction, DRM, private content): a plain error message — "This track couldn't be imported. Try uploading an audio file instead." No technical detail shown.

#### What this does not do

- No in-app Spotify or YouTube player — the track is extracted and stored locally in the session
- No library of linked streaming accounts — this is a one-off paste per session, not an integration
- No saved "sources" — the URL is not stored after extraction is complete

#### Acceptance criteria

- [ ] A valid Spotify or YouTube URL pasted into the music input triggers a preview within 2 seconds
- [ ] Confirmed extraction attaches the track and begins waveform rendering within 30 seconds on a standard connection
- [ ] Failed extractions show a plain error message with a file upload fallback — no crash, no empty state
- [ ] The flow requires no account connection or OAuth

#### Technical notes

- Extraction via `yt-dlp` (server-side job); output format: `m4a`, 128kbps — sufficient for choreography use
- Extraction job enqueued to existing background worker queue; result stored in Supabase Storage under `music_tracks` bucket
- `music_tracks` table gains a `source_url` nullable column for provenance only — not used after extraction
- Preview metadata (title, artist, duration) fetched via `yt-dlp --dump-json` before full extraction
- DRM / region errors caught at preview stage; user sees fallback message before committing

---

### 1.4e  Music Intelligence Track — Chord Progression and Melodic Contour

#### Problem statement

Choreographers already know where the chorus is. What they often don't know — especially when working with K-pop or any genre they didn't grow up with — is *why* a section feels the way it does. Why does the pre-chorus feel like it's about to explode? Why does that last 8-count of the hook feel unresolved? The answer lives in the chord progression and the shape of the melody: a II–V–I that finally lands, a melody that has been climbing for 16 bars and suddenly drops an octave.

This information exists in the music. Most choreographers can hear it but can't name it. Making it visible on the timeline lets choreographers use what they already feel and connect it to specific moments — so a note like "this 8-count wants something that resolves" has a timecode attached to it.

#### Goal

Add an optional timeline track that surfaces two layers of musical information — chord progression and melodic contour — as a visual overlay choreographers can read at a glance, without requiring any music theory knowledge.

#### The track

The Music Intelligence track is an optional user-added track (available via "+ Track" → "Music Intelligence"). It renders as a single lane divided into two visual layers:

```
┌──────────────────────────────────────────────────────────────────────┐
│ 🎼  [  Am  ][  F  ][  C  ][  G  ][  Am  ][  F  ][  C  ][  G  ]     │  ← chord strip
│     ╭──╮                    ╭───╮           ╭──╮                     │
│   ╭╯  ╰──╮          ╭───╮ ╯    ╰╮     ╭──╯  ╰──────╮               │  ← melodic contour
│  ─╯       ╰────────╯    ╰╯      ╰─────╯              ╰──────        │
└──────────────────────────────────────────────────────────────────────┘
...
**Chord strip (top half of the track)**

A sequence of labeled blocks, each spanning the duration of that chord. Block width is proportional to chord duration. The label shows the chord root and quality in plain text: `Am`, `F`, `C`, `G`, `Dm`, `Bm7`, etc. Music theory knowledge is not required to use this — the blocks are visual markers that show *when the harmonic texture changes* more than they communicate what the chord means.

Choreographic use: the moment a chord changes is often the moment a movement phrase should do something — release, accent, shift quality. The strip makes those moments visible without requiring the choreographer to hear them in isolation.

Color coding (subtle, not labels):
- Bright/major-quality chords: slightly warmer tint
- Minor/darker-quality chords: slightly cooler tint
- No legend required — the color is a texture, not a data category

**Melodic contour line (bottom half of the track)**

A smooth line tracing the pitch of the dominant melody (vocals where present, lead instrument otherwise) over time. The line does not show note names or frequencies — only the *shape* of the melody: where it climbs, where it drops, where it plateaus, where it breathes (gaps in the line indicate rests).

Choreographic use: a melody that climbs for 8 bars and then drops is asking for something in the movement — an arrival, a release, a shift in weight. A plateau signals space. The shape of the line gives choreographers a spatial analogy for what the music is doing dynamically, which translates directly into body choices.

#### Interaction

- Tap any chord block → a small tooltip shows the chord name and duration in beats (e.g. "Am — 4 beats"). Tapping also jumps the playhead to that chord's start.
- The melodic contour line is read-only — no interaction required. It scrolls with the timeline.
- Long-press on a chord block → "Pin a note here" — drops a note pin at that chord's start timecode, pre-populated with the chord name as a label (editable). This lets choreographers quickly annotate "I want something to happen when this changes to Dm" without breaking flow.

#### Availability and processing

Music Intelligence analysis runs as part of the same background job as waveform generation and stem separation. It does not require a separate trigger. Results are typically available within 60 seconds of track upload.

If analysis is unavailable or confidence is low (e.g. a track with very dense harmonic content where chord detection accuracy falls below a usable threshold), the track shows a plain waveform fallback with a note: "Music Intelligence unavailable for this track." No error, no broken state.

The track is available to all users. It is an opt-in add-on via "+ Track" — it does not appear unless requested.

#### What this is not

- Not a music theory lesson. The chord names are labels, not curriculum. The tool does not explain what `bVII` means or teach Roman numeral analysis.
- Not a composition tool. Nothing in this track produces, modifies, or suggests musical content.
- Not MIDI. The melodic contour is a visual shape derived from pitch analysis — it is not exported, not editable, and not a MIDI track.

#### Acceptance criteria

- [ ] Music Intelligence track is available via "+ Track" in the additional tracks list
- [ ] Chord strip renders with correct chord labels aligned to the timecode within the same timeline scroll/zoom controller as all other tracks
- [ ] Melodic contour line renders smoothly and updates at the same zoom level as the timeline
- [ ] Tapping a chord block shows a tooltip and jumps the playhead within 200ms
- [ ] Long-press on a chord block drops a pre-labelled note pin in under 2 taps
- [ ] Track shows a plain fallback state (not an error) when analysis is unavailable
- [ ] Adding the track does not affect playback performance — timeline remains ≥ 60fps on mid-range device

#### Technical notes

- Chord analysis: `chord-recognition` model (e.g. `madmom` or equivalent) run server-side alongside Demucs; results stored as `music_intelligence` JSON blob on `music_tracks`: `{ chords: [{ label, start_ms, end_ms, quality }], contour: [{ time_ms, pitch_norm }] }`
- `pitch_norm` is pitch normalized to 0.0–1.0 range (relative to the track's pitch range) — never exposed as Hz or MIDI note number to the UI
- Chord strip rendering: each chord block is an SVG rect with a text label; block width computed from `(end_ms - start_ms) / total_ms * timeline_width` — same pixel math as section markers
- Melodic contour rendering: SVG polyline or smooth bezier path drawn from `contour` data points; gaps (rests) represented as line breaks where confidence falls below threshold
- Color tinting: `quality` field on chord data (`major | minor | dominant | diminished | other`) maps to a CSS variable tint; no user-facing legend
- Note pin from long-press: prefills `annotations.body` with the chord label string; `timecode_ms` set to `chord.start_ms`
- Processing trigger: same job queue as `stem_separation`; both jobs dispatched on `music_track.uploaded` event; `music_intelligence_status` enum column on `music_tracks` (`pending | processing | complete | failed`)
...
- [ ] A pasted Spotify or YouTube URL triggers a track preview within 2 seconds
- [ ] Stem control absent when feature disabled; functional when enabled
- [ ] Creating a second loop region does not remove the first
- [ ] Recording indicator does not change state at loop boundaries
- [ ] "New take" starts a new clip within 300ms with no gap
...
- **Stem focus:** Demucs v4 server-side; results cached as `stems/{track_id}/{stem_name}.m4a`; `active_stem` nullable on `sessions`; `stem_focus_enabled` boolean on `user_preferences` (default false)
- **Music source import:** `yt-dlp` server-side extraction job; output `m4a` stored in `music_tracks` bucket; `music_tracks.source_url` nullable for provenance
- **Multi-loop:** `loop_regions.is_active` boolean; sequence playback ordered by `start_ms`; takes as `clips` rows with `take_number` + `parent_session_id`; loop-cycle markers as `annotations` with `type = loop_cycle`; `AVCaptureSession` lifecycle independent of `AVPlayer` loop boundary

---

### Phase 2 — Micro-cycle: Loop → Capture → Tag
...
- [ ] A lightweight share link is generated in one tap from any clip with no required fields
- [ ] Viewer form shows exactly one prompt and one text input
...
- Lightweight share: `share_tokens.mode = lightweight`; responses stored in `feedback_comments` with `step = 1`; no new tables required

---

### 2.9  Lightweight Share — "What stayed with you?"

#### Problem statement

Share fear sits at 55% for dancer-makers and is not addressed by Phase 0 or Phase 2. Phase 4 (Structured Collaboration) is the full solution, but it ships later. If dancers have no safe sharing container until Phase 4, a cohort that is actively making will not be sharing — and the feedback loop that drives retention breaks before it forms.

The fear is specific: showing rough work before it is ready, and receiving unstructured opinions in return. The Phase 4 Liz Lerman flow solves this fully. This section provides the minimum version of that container — enough to make sharing feel safe — available from Phase 2 onward.

#### Goal

Give any dancer or choreographer a one-tap way to share a clip with a single constraint on the viewer: they can only respond to "what stayed with you?" — no opinions, no ratings, no open comments. This is not a collaboration tool. It is a safe container for rough work.

#### Flow

From any clip in the session workspace or library:
...
- One tap generates the link — no brief, no version label, no configuration
- The viewer sees the clip and a single free-text prompt: "Describe something specific you noticed or remember."
- No rating. No opinion field. No other input.
- Responses arrive in a simple list under the clip in the Ideas or Notes tab — not a separate inbox

#### Relationship to Phase 4

When Phase 4 ships, this lightweight share mode is not removed. It remains as the default quick-share path. Phase 4's full feedback brief (multiple questions, versioning, Step 4 opinions) is accessed via "Advanced share" for sessions where the choreographer wants structured critique. The two modes coexist: lightweight share for rough ideas, structured brief for deliberate feedback sessions.

#### Acceptance criteria

- [ ] A share link can be generated in one tap from any clip — no required fields
- [ ] The viewer form shows exactly one prompt and one text input — no other UI
- [ ] Responses appear under the clip within the existing session view — no new screen required
- [ ] The lightweight share link works without the viewer having a Roam account

#### Technical notes

- Share link generated as a `share_tokens` record with `mode = 'lightweight'` and no `brief` JSONB payload
- Viewer page renders the clip and a single `feedback_comments` input with `step = 1` hardcoded
- Responses stored in the existing `feedback_comments` table — no new table required
- Response display: a collapsed section at the bottom of the clip detail view, "N responses — what stayed with them"
- When Phase 4 ships: `mode = 'lightweight'` links continue to work unchanged; `mode = 'structured'` links use the full brief flow

---


### Phase 3 — Cleaning / Review Mode
...
11. **Stem default configuration:** Should Stem Focus remember the last used stem configuration across sessions, or reset to full mix at the start of every session? Persistent state could be useful for choreographers who always isolate drums, but could surprise users who forgot they had muted something.
12. **Music Intelligence track visibility:** Should the chord strip and melodic contour be shown as two separate sub-rows within the track, or as a single combined lane? Two rows are clearer but take more vertical space on smaller devices. Test on both 5.5" and 6.7" screens before deciding.
13. **Chord label verbosity:** Plain chord labels (`Am`, `F`) are low-friction but may feel meaningless to choreographers with no music background. Consider an optional "interpretation mode" that replaces chord labels with movement-relevant descriptors (`tense`, `open`, `landing`, `lifting`) — test with non-musician choreographers to see if this adds value or adds confusion.
