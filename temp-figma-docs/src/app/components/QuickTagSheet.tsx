...
export default function QuickTagSheet({ isOpen, onClose, section = "Chorus", timecode = "0:42" }: QuickTagSheetProps) {
      ...
      >
        ...
        <div className="p-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {typeOptions.map((option, index) => (
                <button
                  ...
                  style={{
                    backgroundColor: index === 0 ? 'var(--accent-primary)' : 'var(--surface-raised)',
                    border: index === 0 ? 'none' : '1px solid var(--border-subtle)',
                    color: index === 0 ? 'var(--surface-base)' : 'var(--text-primary)',
                    fontSize: '12px',
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {feelOptions.map((option, index) => (
                <button
                  key={option}
                  className="px-3.5 h-9 rounded-full whitespace-nowrap flex-shrink-0"
                  style={{
                    backgroundColor: index === 0 ? 'var(--accent-warm)' : 'var(--surface-raised)',
                    border: index === 0 ? 'none' : '1px solid var(--border-subtle)',
                    color: 'var(--text-primary)',
                    fontSize: '12px',
                    fontWeight: index === 0 ? 600 : 400,
          ...
          <button
            onClick={onClose}
            className="w-full h-13 rounded-lg flex items-center justify-center gap-2"
            style={{
