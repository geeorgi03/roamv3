...
app.patch("/make-server-837ff822/inbox/:clipId/assign", async (c) => {
  try {
    ...
    const assignedClip = { ...(clip as object), sessionId };
...
// Generate a share token for a clip (authenticated)
app.post("/make-server-837ff822/sessions/:sessionId/clips/:clipId/share", async (c) => {
  try {
    ...
    const existingToken = await kv.get(`share:clip:${clipId}:token`);
    if (existingToken) {
      return c.json({ token: existingToken });
    }

    const token = crypto.randomUUID().replace(/-/g, '');
    await kv.set(`share:token:${token}`, { clipId, sessionId, userId, createdAt: new Date().toISOString() });
    await kv.set(`share:clip:${clipId}:token`, token);

    console.log(`Share token created for clip ${clipId}: ${token}`);
    return c.json({ token });
  } catch (error) {
...
// Public: get clip info via share token (no auth required)
app.get("/make-server-837ff822/share/:token", async (c) => {
  try {
    ...
    const { clipId, sessionId, userId } = shareData as any;
    const clip = await kv.get(`clip:${userId}:${sessionId}:${clipId}`);
    if (!clip) return c.json({ error: 'Clip not found' }, 404);

    return c.json({ clip, sessionId });
  } catch (error) {
    console.log(`Error fetching shared clip: ${error}`);
    return c.json({ error: 'Failed to load shared clip' }, 500);
  }
});

// Public: submit a viewer response (no auth required)
app.post("/make-server-837ff822/share/:token/response", async (c) => {
  try {
    ...
    if (!text?.trim()) return c.json({ error: 'Response text is required' }, 400);
...
// Get all responses for a clip (authenticated — choreographer view)
