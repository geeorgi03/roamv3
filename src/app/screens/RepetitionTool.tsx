import { ArrowLeft, SkipBack, SkipForward, Plus, Repeat } from "lucide-react";
...
export default function RepetitionTool() {
  ...
  const { id: sessionId, regionId } = useParams<{ id: string; regionId: string }>();
  const { session, loops, updateLoop, loading, error } = useSessionData(sessionId || null);
  const loop = useMemo(() => loops.find((l) => l.id === regionId), [loops, regionId]);
  ...
  const savedRegions = useMemo(() => {
    return loops.map((l, idx) => ({
      name: l.name,
      range: `${formatTime(l.startTime)}–${formatTime(l.endTime)}`,
      count: l.repeatCount || "∞",
      color: `var(--track-${(idx % 4) + 1})`,
      active: l.id === regionId,
      id: l.id,
    }));
  }, [loops, regionId]);

  const duration = session?.duration || 0;
  const startPct = loop && duration > 0 ? (loop.startTime / duration) * 100 : 0;
  const endPct = loop && duration > 0 ? (loop.endTime / duration) * 100 : 0;

  useEffect(() => {
    if (loop?.repeatCount) setRepeatCount(loop.repeatCount);
  }, [loop?.repeatCount]);
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3" style={{ backgroundColor: "var(--surface-base)" }}>
        <div style={{ color: "var(--text-secondary)", fontFamily: "var(--font-body)" }}>{error}</div>
        <button onClick={() => navigate(-1)} style={{ color: "var(--accent-primary)", fontFamily: "var(--font-body)" }}>
          Back
        </button>
      </div>
    );
  ...
  if (!loop) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3" style={{ backgroundColor: "var(--surface-base)" }}>
        <div style={{ color: "var(--text-secondary)", fontFamily: "var(--font-body)" }}>Region not found</div>
        <button onClick={() => navigate(-1)} style={{ color: "var(--accent-primary)", fontFamily: "var(--font-body)" }}>
          Back
        </button>
      </div>
      ...
      >
        ...
        <div className="h-32 flex items-end gap-0.5">
          {Array.from({ length: 100 }).map((_, i) => {
            ...
            const pct = (i / 100) * 100;
            const isInRegion = pct >= startPct && pct <= endPct;
            return (
              <div
                key={i}
